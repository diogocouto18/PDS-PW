...
-- CreateTable
CREATE TABLE `RifaComprada` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `id_utilizador` INTEGER NOT NULL,
    `id_pagamento` INTEGER NOT NULL,
    `estado` ENUM('Vencedor', 'SegundoLugar', 'TerceiroLugar', 'Perdedor', 'Pendente') NOT NULL DEFAULT 'Pendente',

    INDEX `id_utilizador`(`id_utilizador`),
    INDEX `id_pagamento`(`id_pagamento`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `RifaComprada` ADD CONSTRAINT `rifacomprada_ibfk_1` FOREIGN KEY (`id_utilizador`) REFERENCES `Utilizador`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

-- AddForeignKey
ALTER TABLE `RifaComprada` ADD CONSTRAINT `rifacomprada_ibfk_2` FOREIGN KEY (`id_pagamento`) REFERENCES `Pagamento`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- AddForeignKey
ALTER TABLE `Pagamento` ADD CONSTRAINT `pagamento_ibfk_2` FOREIGN KEY (`id_sorteioRifas`) REFERENCES `SorteioRifas`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
